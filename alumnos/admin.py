...
class PagoInline(admin.TabularInline):
    ...
    fields = (
        'inscripcion', # 👇 AHORA MUESTRA EL CURSO
        'metodo_pago',
        'monto_total',
        'cantidad_cuotas',
        'fecha_pago',
...
class PagoAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'alumno',
        'inscripcion', # 👇 AHORA MUESTRA EL CURSO EN LA TABLA
        'metodo_pago',
        'ver_monto_total',
        'cantidad_cuotas',
        'fecha_pago',
    ...
    # SE ELIMINÓ: exclude = ('inscripcion',) para que el campo Curso aparezca
    readonly_fields = ('info_alumno_detalle',)

    def get_readonly_fields(self, request, obj=None):
        if obj:
            # 👇 AHORA SE BLOQUEA TAMBIÉN LA INSCRIPCIÓN AL GUARDAR
    ...
    def info_alumno_detalle(self, obj):
        if obj and obj.alumno:
            # 👇 AHORA INCLUYE EL BOTÓN PARA AGREGAR NUEVO PAGO AL MISMO ALUMNO
            return format_html(
    ...
    info_alumno_detalle.short_description = "Datos del Alumno y Acciones"
            ...
            if isinstance(instance, Cuota) and instance.pk:
                ...
                if cuota_antigua.estado != 'PAGADA' and instance.estado == 'PAGADA':
                    Auditoria.objects.create(
                        ...
                        descripcion=(
                            f"ATENCIÓN: Se marcó como PAGADA la Cuota {instance.numero_cuota} "
                            f"de {formato_clp(instance.monto)} correspondiente a {instance.pago.alumno}."
                        )
                    )
            instance.save()
        formset.save_m2m()
